...
<v-dialog v-model="show" v-if="show" scrollable fullscreen>
 <v-card>
  ...
  <div class="ios-vcard-text-workaround"><div class="infodetails" id="info-page">
   ...
   <ul>
    <template v-for="(plug, index) in plugins.details"><li v-if="'downloading'==pluginStatus || !updates.names.has(plug.name)">{{plug.title}} {{plug.version}}<v-btn flat icon class="info-help-icon" @click="pluginInfo(plug)"><v-icon small>help_outline</v-icon></v-btn></li></template>
   </ul>
   <div class="dialog-padding"></div>

...
<v-layout row wrap :class="'vol-control-'+layout">
 <v-flex xs12 v-if="layout==0"><p class="vol-text link-item noselect" v-bind:class="{'pulse':!noPlayer && value==0 && playing}" @click.middle="toggleMute" v-longpress.prevent.stop="toggleMuteLabel">{{value|displayVal(dvc, name)}}</p></v-flex>
 <v-flex :disabled="VOL_HIDDEN==dvc" xs12>
  <v-layout>
   ...
   <p v-if="layout==1" class="vol-full-label" v-bind:class="{'link-item-ct':coloredToolbars,'link-item':!coloredToolbars,'disabled':noPlayer,'dimmed':muted,'pulse':!noPlayer && value==0 && playing}" @click.middle="toggleMute" v-longpress.prevent.stop="toggleMuteLabel" id="vol-label">{{value|displayVal(dvc)}}</p>
   <p v-else-if="layout==2 && VOL_STD==dvc" class="pmgr-vol link-item noselect" v-bind:class="{'pulse':value==0 && playing}" @click.middle="toggleMute" v-longpress.prevent.stop="toggleMuteLabel">{{value|displayVal(dvc)}}</p>
  </v-layout>
 </v-flex>
</v-layout>
    `,

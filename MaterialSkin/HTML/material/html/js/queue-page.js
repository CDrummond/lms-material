...
<div> 
 ...
 <v-list class="lms-list-sub bgnd-cover" id="queue-list">
  ...
  <!-- <template><recycle-list :items="items" :item-height="56" page-mode><div slot-scope="{item, index}"> -->
   <v-list-tile :key="item.title" avatar v-bind:class="{'pq-current': index==currentIndex}" :id="'track'+index" @dragstart="dragStart(index, $event)" @dragover="dragOver($event)" @drop="drop(index, $event)" draggable @click="select(item, index)">
...
`,
    ...
    created() {
        ...
        this.playerStatus= { current: { artwork_url:undefined, coverid: undefined} };
        ...
        bus.$on('playerStatus', function(playerStatus) {
            ...
            if (playerStatus.playlist.count == 0) {
                ...
                if (this.playerStatus.current.artwork_url) {
                    coverUrl=resolveImage(null, this.playerStatus.current.artwork_url);
                }
                if (undefined==coverUrl && this.playerStatus.current.coverid) {
                    coverUrl=lmsServerAddress+"/music/"+this.playerStatus.current.coverid+"/cover.jpg";
                }

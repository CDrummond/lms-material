...
Vue.component('lms-advancedsearch-dialog', {
    template: `
<v-dialog v-model="show" v-if="show" scrollable max-width="900">
 <v-card>
...
`,
    ...
    mounted() {
        bus.$on('advancedsearch.open', function(reset, libId) {
            ...
            if (reset) {
                ...
                this.params.me_discsubtitle= {val:undefined, op:"LIKE"};
    ...
    methods: {
        ...
        search() {
            this.searching = true;
            var ops = ['me_titlesearch', 'contributor_namesearch', 'album_titlesearch', 'me_discsubtitle', 'me_subtitle', 'work_titlesearch', 'album_release_type', 'secs', 'tracknum', 'year', 'persistent_playcount', 'persistent_rating', 'timestamp', 'url', 'filesize', 'comments_value', 'lyrics'];
        ...
        emitResults(results, total, command) {
            ...
            let resp = buildSearchResp(this);
            bus.$emit('advSearchResults', item, {command:command, params:[]},
                      { items:resp.items, itemCustomActions:resp.itemCustomActions, baseActions:[], canUseGrid: false, jumplist:[], subtitle:i18np("1 Item", "%1 Items", total)});
            this.searching = false;

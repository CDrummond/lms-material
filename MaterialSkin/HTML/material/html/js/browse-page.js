 ...
 <v-menu v-model="menu.show" :position-x="menu.x" :position-y="menu.y">
  ...
  <v-list v-else-if="menu.item">
   <template v-for="(action, index) in menu.itemMenu">
    ...
    <v-list-tile v-else-if="action==BR_COPY_ACTION ? queueSelection : action==MOVE_HERE_ACTION ? (selection.size>0 && !menu.item.selected) : action==RATING_ACTION ? undefined!=ratingsPlugin : (action!=DOWNLOAD_ACTION || queryParams.appDownload && 0==selection.size)" @click="itemAction(action, menu.item, menu.index, $event)">
     <v-list-tile-avatar v-if="menuIcons">

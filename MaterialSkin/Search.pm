...
sub advancedSearch {
	...
	for my $key (sort keys %$params) {
		...
		if ($key =~ /\.op$/) {
			...
			$newKey =~ s/_(rating|playcount|value|discsubtitle|subtitle|titlesearch|namesearch|)\b/\.$1/;
		...
		if ($newKey =~ s/(.+)_(titlesearch|namesearch|value|discsubtitle|subtitle|release_type|)$/$1\.$2/) {
			$joins{$1}++ if $1 ne 'me';

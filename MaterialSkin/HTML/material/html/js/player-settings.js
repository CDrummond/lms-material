  ...
  <v-card>
   <v-card-title class="settings-title">
    <v-toolbar app-data class="dialog-toolbar" @mousedown="mouseDown" id="playersettings-toolbar">
     ...
     <v-btn v-if="IS_IOS" flat icon @click="close(false)" :title="ttShortcutStr(i18n('Go back'), 'esc')"><v-icon v-bind:class="{'apple-back':IS_APPLE}">{{BACK_ARROW}}</v-icon></v-btn>
     <v-btn v-else flat icon v-longpress:stop="close" :title="ttShortcutStr(i18n('Go back'), 'esc')"><v-icon>{{BACK_ARROW}}</v-icon></v-btn>
    ...
    <v-header class="dialog-section-header" v-if="unlockAll" id="alarms">{{i18n('Alarms')}}</v-header>
     ...
     <v-list-tile v-if="showSync" class="other-setting">
      <v-list-tile-content>
       ...
       <v-list-tile-sub-title>{{isSynced ? i18n('Synchronized with other players.') : i18n('Not currently synchronized with any other player.')}}</v-list-tile-sub-title>
      </v-list-tile-content>
     </v-list-tile>
